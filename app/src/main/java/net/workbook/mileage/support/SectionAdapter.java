...
public class SectionAdapter<T> extends BaseAdapter implements Finals
{
	...
	public SectionAdapter<Job> filterSections(ArrayList<Job> finalObjects,CharSequence charSequence)
    {
    	if (charSequence.equals(NOTHING))
    	{
    		...
    		return new SectionAdapter<Job>(mContext, mListAdapter, R.layout.section, R.id.list_item_section_text, customerSectionizer);
    	}
    	else
    	{
    		...
    		{
    			String lowerJobName = job.getJobName().toLowerCase();
    			String lowerCustomerName = job.getCustomerName().toLowerCase();
    			if (lowerJobName.contains(charSequence) || lowerCustomerName.contains(charSequence) || ((String)(job.getJobId()+NOTHING)).contains(charSequence))
    			{
    				objects.add(job);
    			}
    		}
    		ArrayAdapter<Object> jobs = new ArrayAdapter<Object>(mContext, R.layout.row_simple, objects);
    		Sectionizer<Job> customerSectionizer = new Sectionizer<Job>() 
					{		
						@Override
						public String getSectionTitleForItem(Job instance) 
			...
			return new SectionAdapter<Job>(mContext, jobs, R.layout.section, R.id.list_item_section_text,customerSectionizer);
    	}
    }
